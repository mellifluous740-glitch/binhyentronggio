...
<section id="thong-bao"><h2>Thông báo</h2>{% assign notices = site.announcements | sort: 'date' | reverse %}{% for notice in notices %}<article class="notice"><p class="eyebrow">{{ notice.tag }} · {{ notice.date }}</p><h3><a href="{{ notice.url | relative_url }}">{{ notice.title }}</a></h3><p>{{ notice.content | strip_html | truncate: 220 }}</p></article>{% endfor %}</section>
